...
export default function Sidebar() {
        ...
        },
        {
            ...
            icon: (
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0 20V2C0 1.45 0.195833 0.979167 0.5875 0.5875C0.979167 0.195833 1.45 0 2 0H18C18.55 0 19.0208 0.195833 19.4125 0.5875C19.8042 0.979167 20 1.45 20 2V14C20 14.55 19.8042 15.0208 19.4125 15.4125C19.0208 15.8042 18.55 16 18 16H4L0 20ZM3.15 14H18V2H2V15.125L3.15 14ZM2 14V2V14Z" fill="white" />
                </svg>
            ),
        },
        {
    ...
    return (
        <aside className="w-64 h-screen sticky top-0 bg-[#134E4A]/70 flex flex-col justify-between py-8 px-6 z-50">
            <div>
                <div className="mb-8">
                    <h1 className="text-[24px] font-black text-white leading-8">
                        Tomoio
                    </h1>
                    <p className="text-[12px] font-medium text-white/70 leading-4 tracking-[0.3px]">
                        プレミアム・マッチング
                    </p>
                </div>
                    ...
                    {navLinks.map((link) => {
                        ...
                        return (
                            <Link
                                ...
                                onFocus={() => prefetchForPath(link.path)}
                                className={`relative flex items-center gap-3 p-[12px_16px] rounded-lg transition-colors ${isActive ? "bg-white/20" : "hover:bg-white/10"
                                    }`}
                            >
                                <div className={isActive ? "text-white" : "text-white/80"}>
                                    {link.icon}
                                </div>
                                <span className={`text-[14px] ${isActive ? "font-bold text-white" : "font-medium text-white/80"}`}>
                                    {link.name}
                                </span>

                                {isActive && (
                                    <div className="absolute right-2 top-1/2 -translate-y-1/2 w-1 h-4 bg-white rounded-full"></div>
                                )}
                            </Link>
                        );
                    })}
