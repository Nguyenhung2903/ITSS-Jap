...
export async function getPostsAction(groupId: number, page = 1) {
    try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/posts?groupId=${groupId}&page=${page}&limit=10`, {
...
export async function likePostAction(postId: number) {
    try {
        const cookieStore = await cookies();
        const token = cookieStore.get("tomoio_token")?.value;
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/posts/like`, {
            method: "POST",
...
export async function commentPostAction(postId: number, content: string) {
    try {
        const cookieStore = await cookies();
        const token = cookieStore.get("tomoio_token")?.value;
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/posts/comment`, {
            method: "POST",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
...
export async function unlikePostAction(postId: number) {
    try {
        const cookieStore = await cookies();
        const token = cookieStore.get("tomoio_token")?.value;
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/posts/unlike`, {
            method: "POST",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
            body: JSON.stringify({ postId })
        });
...
export async function getCommentsAction(postId: number, page = 1) {
    try {
        ...
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/posts/${postId}/comments?page=${page}&limit=20`,
            {
                headers: { "Authorization": `Bearer ${token}` },
                cache: "no-store"
            }
